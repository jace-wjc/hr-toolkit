...
class QtEntrypointTests(unittest.TestCase):
    ...
    def test_file_drop_targets_are_separate_and_do_not_add_polling(self) -> None:
        ...
        self.assertIn("dropTarget.role, dropTarget.urlsFor(drag)", target)
        self.assertIn("finishDropPreview(dropTarget.feedback.token, dropTarget.role, urls)", target)
        self.assertIn("dropTarget.feedback.accepted || dropTarget.feedback.pending", target)
