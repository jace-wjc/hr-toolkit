...
def tutorial_lines(tool_id: str, mode: str | None = None) -> list[TutorialLine]:
    ...
    if tool_id == "salary_merge":
        return [
            ...
            ("点击“开始合并”后，结果会保存到当前工作项目，源文件请自行保管。", None),
    ...
    if tool_id == "personnel_change_merge":
        if mode == "roster":
            return [
                ...
                ("点击“更新花名册”后，结果会保存到当前工作项目，源文件请自行保管。", None),
        ...
        return [
            ...
            ("点击“开始汇总”后，结果会保存到当前工作项目，源文件请自行保管。", None),
    ...
    if tool_id == "folder_rename":
        return [
            ...
            ("完整预览：查看全部原名称、新名称、相对路径和问题；可搜索、筛选、排除项目，直接编辑每项名称。全部已选项目校验通过后执行确认方案。", None),
            ("按 Excel 人名顺序批量重命名：先按顺序生成方案；在完整预览中用上移/下移调整姓名分配，文件位置和扩展名不变。确认后不再按 Excel 重排。", None),
            ("按 Excel 原文件名匹配：准备“原文件名”和“新名称”两列，按完整原文件名匹配而非行序；未匹配项目默认排除，可在预览中补充。", None),
            ...
            ("替换指定文字：按所选文件类型批量替换名称中的原文字，例如“劳动合同”改为“资金合同”；文件扩展名和内容保持不变，区分大小写并替换所有匹配文字。", None),
            ("文字替换只处理第一层，沿用类型筛选并忽略以 . 或 ~$ 开头的项目；“全部”包含任意扩展名文件。目标重名或无效时，先在完整预览中修正或排除；不会覆盖未参与改名的项目。", "warning"),
            ("规范名称格式：可去除首尾空格、合并空格，按需统一横线和下划线；不自动删除“作废、补签”等业务文字。", None),
            ("完成后在处理结果中保存改名记录和 CSV 清单。执行中出错或停止时尝试安全恢复；若无法全部恢复，请保留结果目录和记录，不要手动删除临时项目。", "warning"),
