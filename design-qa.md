...
- 操作限制提前至更新准备、下载和校验阶段：所有业务工具的主执行入口及已有确认窗口的后续执行入口均拦截，显示等待提示；下载失败或取消后恢复，下载完成继续等待重启。仅检查版本不锁定，已运行任务仍可停止。同步 controller 用例与 QML 源码断言；本轮未运行测试套件或真实更新。
- 最新交互调整：自动检查发现支持自动更新的新版本时直接在左下角下载，不弹确认框；手动检查保留原更新说明弹窗，确认后在左下角下载。两种方式下载完成均等待点击“重启以更新”；仅手动检查在无新版本时弹窗，不再创建独立下载进度弹窗。仅支持外部手动下载的更新清单仍保留提示，不在后台擅自打开浏览器。已补充回归用例源码；本次仅做修改 Python 文件的最小语法检查，未运行测试套件或真实安装。
- 本轮按用户补充要求修订：取消独立绿色进度条；卡片内以浅灰蓝半透明填充从底部向上增长，260毫秒过渡，右侧显示真实下载百分比。卡片固定48逻辑像素高，保留圆角、浅阴影和内侧高光边线；不使用模糊、波浪、ShaderEffect 或额外依赖。
- 图标修订：`components/update-feather.png` 直接提取自用户参考截图的 `(18,784,28,28)` 区域，并去除白底，使用参考中的几何线框羽毛；不再使用工具 Logo，也不宣称该图标来自 Phosphor。现有工具底部品牌 Logo 不变。
- 本轮实现证据：同一路径的 ready/downloading 全图及局部截图已重新生成，替代上一轮卡片截图；46% 为隔离预览状态。原图羽毛与实现局部在同次图像输入中核对。现有逻辑未调整，只有视觉变化；本机 QML 实际加载及截图无布局警告。

- Source visual truth：`/var/folders/ng/s5d0j44s06z6zdc_qr9ylpv80000gn/T/codex-clipboard-0bd35279-eb32-426d-8d1b-53877a109e99.png`（1407×873），仅匹配左下角更新卡片；采用参考羽毛图标和中文文案。
- 实现截图：`/tmp/hr-background-update.2jdtQh/ready.png`、`downloading.png`（Mac 原生 QML 客户区1400×780，1×）；局部 `ready-card.png`、`downloading-card.png`（248×160）。版本0.9.4及46%为隔离预览数据，未联网、下载或安装真实更新。
- 全图与局部对照：同次图像输入查看参考和实现，忽略业务页面、操作系统标题栏及视口高度差异；聚焦约248像素侧栏底部的同一组件。卡片边距8、高48；下载状态采用本轮用户要求的整卡填充。
- 字体与内容：复用应用字体，中文标题12像素、版本号10像素；标题、版本号、箭头不截断；右侧处理按钮旁显示更新提示。未改业务字段。
- 颜色与资产：白底、细边框、浅阴影与10像素圆角；下载填充使用低对比灰蓝色。羽毛来自用户截图，右箭头为 Phosphor 官方 SVG 转96像素 PNG，运行时只读 PNG。为兼容 Win7，使用低成本静态浅阴影，不引入模糊特效依赖；不声称和 Claude 的阴影像素完全相同。
- 行为边界：截图确认就绪时主处理按钮变灰，其他常规控件保持原有状态；下载时不弹模态更新窗口。真实点击重启安装、Win7/Win11、缓存跨进程恢复尚未实测；回归用例已补充但按项目验证边界未执行测试套件。
- 预览中发现更新历史初始化时 entries 尚未赋值会报错，增加空值保护后重新加载不再出现该 QML 错误。退出时仍有 macOS 输入法 mach port 日志，与更新卡片无关。
- Findings：本机更新卡片两种状态未发现剩余 P0/P1/P2 视觉问题。外观检查通过不代表安装链路通过；发布前仍需 Windows 实机验收。
...
- 实现截图：`/tmp/hr-sidebar.N2JrQX/expanded.png`，Mac 原生 QML，1400×780、1×，社保页面、侧栏固定展开、未打开项目。
- 比较范围：只匹配背景色，不复制 Claude 的布局、业务内容或控件；两张截图在同次图像输入中完整对照。视口不同，不据此判断尺寸或字体差异；纯色背景无需缩放或局部裁剪，另用像素取样验证。
- 取色：参考左侧空白区域为 `#FBFBF9`，右侧为 `#FCFCFB`；实现中左右背景及对应顶部区域四个取样点均与目标一致。
- 字体与排版、布局间距、图标资产和文案：全部保留当前 HR 界面；仅背景配色及左上方随侧栏移动的底色层变化。卡片、输入框、选中与悬停色均不变。
- Findings：本次背景配色范围未发现 P0/P1/P2 问题；顶部与侧栏连续，无额外色块断层。首轮比较通过，无后续视觉修正。
- 验证：实际 QML 加载及 Mac 截图取样完成。未运行测试套件、构建或发布；未验证 Windows 实机、侧栏动画状态。退出预览时出现 macOS 输入法服务 mach port 日志，不是 QML 加载错误。
- Implementation checklist：主背景改色；侧栏改色；左侧顶部同色且绑定侧栏位置与可见性；不改业务逻辑。
